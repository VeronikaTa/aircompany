...
public class AirportTest {
    ...
    @Test
    public void getHasAtLeastOneTransportMilitaryPlane() {
        Airport airport = new Airport(planes);
        List<MilitaryPlane> transportMilitaryPlanes = airport.getTransportMilitaryPlanes();
        Assert.assertNotNull(transportMilitaryPlanes);
        Assert.assertTrue(transportMilitaryPlanes.stream().anyMatch(militaryPlane -> militaryPlane.getType() == MilitaryType.TRANSPORT));
    }

    @Test
    public void testAssertPassengerPlaneWithMaxCapacity() {
        Airport airport = new Airport(planes);
    ...
    @Test
    public void testPlanesAreSortedByMaximumCapacity() {
        Airport airport = new Airport(planes);
        airport.sortByMaxLoadCapacity();
        List<? extends Plane> planesSortedByMaxLoadCapacity = airport.getPlanesInAirport();

        for(int i = 0; i < planesSortedByMaxLoadCapacity.size()-1; i++){
            Assert.assertTrue(planesSortedByMaxLoadCapacity.get(i)
                    .getMinLoadCapacity() <= planesSortedByMaxLoadCapacity.get(i+1)
                    .getMinLoadCapacity());
        }
    }
